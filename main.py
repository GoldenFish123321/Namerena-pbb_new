...
def _validate_config(cfg: dict):
    ...
    if pr_cfg is not None:
        ...
        for i, pr in enumerate(pr_cfg):
            ...
            # end == 0 且 start == 0: 零长度区间
            if end == 0 and start == 0:
                pass
            elif end == -1:
                pass  # 几乎无限 — engine 层会展开
            elif end <= start:
                _die(f"enumeration.prefix_ranges[{i}].end 必须 > start (或 [0,0]=零长度, -1=无限), 当前: end={end} start={start}")
            total_cnt += cnt
...
def main():
    print(f"[main] Version: {VERSION}", file=sys.stderr)
